...
async function refreshFormToken( form, root ) {
	const tokenInput = form.querySelector( '[name="token"]' );
	if ( ! tokenInput || ! RRZEFormular.tokenUrl ) {
		return;
	}

	const formRoot = root || form.closest( '.rrze-formular' );
	const response = await fetch( RRZEFormular.tokenUrl, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
		},
		body: JSON.stringify( {
			...
			postId: parseInt( formRoot?.dataset.postId || '0', 10 ),
		} ),
	} );
